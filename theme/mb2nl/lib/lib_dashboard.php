...
function theme_mb2nl_dashboard()
{
	...
	foreach ( $tabs as $tab )
	{
		...
		$output .= '<li class="tab-item item-' . $tab['id'] . '">';
		$output .= '<button class="themereset' . $cls . '" type="button" data-id="' . $tab['id'] . '">' . $tab['name'] . '</button>';
		$output .= '</li>';
	}
...
function theme_mb2nl_dashboard_block($tab, $block)
{
	...
	$output .= '<div class="block-item block-item-' . $block['id'] . $clscontnt . $clsvalue . $clsvalue3 . ' col-' . $block['col'] . '">';
	...
	$output .= '<span class="sr-only">' . $block['name'] . '</span>';

	$output .= '<div class="block-value-item">';
	...
	{
		$output .= '<div class="block-value-item">';
		$output .= '<span class="value">' . $block['value2'] . '</span>';
...
function theme_mb2nl_dashboard_blocks()
{
	...
	$blocks = array(
		...
		array(
			...
			'value2' => theme_mb2nl_dashboard_activeusers(),
			'value3' => '',
			'suffix' => get_string('allusers','theme_mb2nl'),
			'suffix2' => get_string('activeusers', 'theme_mb2nl'),
			'suffix3' => '',
		...
		array(
			...
			'value2' => theme_mb2nl_dashboard_newusers(3),
			'value3' => '',
			'suffix' => get_string('allusers','theme_mb2nl'),
			'suffix2' => get_string('lastdays', 'theme_mb2nl', 3),
			'suffix3' => '',
			'more' => '',
			'content' => '',
			'data' => ''
		...
		array(
			...
			'value2' => theme_mb2nl_dashboard_students(false,true),
			'value3' => '',
			'suffix' => get_string('allusers','theme_mb2nl'),
			'suffix2' => get_string('activeusers','theme_mb2nl'),
			'suffix3' => '',
		...
		array(
			...
			'user' => 'admin',
			'value' => theme_mb2nl_dashboard_students(true),
 ...
 *
 */
function theme_mb2nl_dashboard_newusers($days = false)
{
    global $DB;

	$newcreated = theme_mb2nl_dashboard_user_new_time($days);
 ...
 *
 */
function theme_mb2nl_dashboard_onlineusers($time = 5)
{
    global $DB;
	...
	//$sql = 'SELECT COUNT(u.id) FROM {user} u JOIN {role_assignments} ra ON ra.userid = u.id WHERE u.lastaccess BETWEEN';
	$sql = 'SELECT COUNT(u.id) FROM {user} u WHERE u.lastaccess BETWEEN';
	$sql .= ' ' . $onlinestart;
	$sql .= ' AND ' .  $timefinish;
	//$sql .= ' AND ra.roleid = ' . theme_mb2nl_get_user_role_id(true);
