...
function theme_mb2nl_blog_template_item( $post, $options, $super = false )
{
	...
	if ( $options['desc'] && $introtext )
	{
		...
		$output .= '<a class="mb2-pb-btn typelink fwbold" href="' . $postlink . '">' . get_string('continuereading', 'theme_mb2nl') . '</a>';
		$output .= '</div>';		
	}
...
function theme_mb2nl_get_announcements($opts)
{
	...
	return $DB->get_records_sql( $sqlquery . $sqlwhere . $sqlorder, array(), 0, $opts['limit'] ); 
...
function theme_mb2nl_get_announcements_tmpl($opts)
{
	...
	{
		$announcements = array('name'=>get_string('nonews','forum'), 'id'=>0);
	...
	{
		$link = isset($opts['pb']) || $a->id == 0 ? '#' : new moodle_url('/mod/forum/discuss.php', array('d'=>$a->id));
