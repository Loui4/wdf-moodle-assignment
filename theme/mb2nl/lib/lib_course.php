...
function theme_mb2nl_course_boxes($cls = 'bars')
{
	...
	foreach( $links as $item )
	{
		...
		$output .= '<a href="' . $item['link'] . '" class="' . $item['class'] . $acls . '" title="' . $item['title'] . '">';
...
function theme_mb2nl_get_section_activities( $sectionid = 0, $label = false, $onlyuservisible = true )
{
	...
	foreach ( $modinfo->cms as $cm )
	{
		...
		$mod['restriction'] = theme_mb2nl_module_has_restrictions($cm);
...
function theme_mb2nl_section_module_list( $sectionid, $link = false, $active = false, $uservisible = true )
{
	...
	foreach ( $modules as $k=>$m )
	{
		...
		elseif ($m['modname'] !== 'label')
		{
			...
			$output .= '<li class="module-item module-' . $m['modname'] . $modactive . $modcomplete . $hiddencls . theme_mb2nl_bsfcls(1,'','','center') . '" data-cmid="' . $m['id'] . '">';
			...
			$output .= $m['restriction'] && $link ? '<div class="course-badges icons' . theme_mb2nl_bsfcls(2) . '"><span class="restriction" aria-label="' . get_string('accessrestrictions', 'availability') . '"><i class="fa fa-lock"></i></span></div>' : '';
			$output .= $link ? '</a>' : '';
			$output .= '</li>';
		}
	}
...
function theme_mb2nl_course_share_list( $id, $title, $blog = false )
{
	...
	foreach ( $links as $k=>$link )
	{
		...
		$output .= '<li class="' . $k . '">';
		$output .= '<a href="' . $islink . '" class="' . $k . '_link' . theme_mb2nl_bsfcls(2,'','center','center') . '" title="' . $link['title'] . '"' . $target . $dataurl . '>';
...
function theme_mb2nl_course_get_share_links( $id, $title, $blog = false )
{
	...
	$links['sharetwitter'] = array( 'title'=>'Twitter', 'link'=>'https://twitter.com/intent/tweet?text=' . urlencode( $title . ' ' . $url ), 'icon'=>'ri-twitter-x-fill' );
 ...
 *
 */
function theme_mb2nl_course_sections_accordion($count = 0)
{
	global $COURSE, $PAGE;
	...
	$output .= '<!-- tsrttttttt --><div class="coursetoc-sectionlist enrol-sectionlist">';
	...
	foreach ( $sections as $section )
	{
		...
		// Section number
		$numcls = $i < 10 ? ' snum' : '';
		$num = '<span class="section-num' . $numcls . theme_mb2nl_tcls('small','normal',1,'') . '">' . get_string('section') . ' ' . $i . '</span>';

		$output .= '<div class="coursetoc-section coursetoc-section-' . $section['num'] . $isactive  . '">';
		$output .= '<div class="coursetoc-section-tite">';
		$output .= '<button class="coursetoc-section-toggle themereset' . $highlightedcls . '" aria-controls="' . $collid. '">';
		$output .= '<span class="section-header' . theme_mb2nl_bsfcls(1,'','between','center') . '">';
		$output .= '<span class="title-text' . theme_mb2nl_bsfcls(2,'','','center') . '">';	
		$output .= $num;
		$output .= '<span class="text">' . format_text( $section['name'], FORMAT_HTML );
		$output .= $viewhidden ? theme_mb2nl_section_badges($section, true) : '';
		$output .= '</span>'; // text		
		$output .= '</span>'; // title-text		
		$output .= '<span class="toggle-icon"></span>';

		$output .= '</span>'; // section-header
		$output .= '</button>';
...
function theme_mb2nl_course_tabs($layout = 'ver')
{
	...
	$output .= '<button type="button" class="themereset course-nav-list-item-toggle' . theme_mb2nl_bsfcls(1,'wrap','between','center') . '" aria-controls="course-section-menu-list">';
	...
	foreach( $sections as $section )
	{
		...
		$output .= '<button type="button" class="themereset course-nav-button' . $activecls . $completecls . $highlightedcls . theme_mb2nl_bsfcls(1,'row','','center') . '" aria-controls="course-nav-section-' . $section['num'] . '">';
...
function theme_mb2nl_section_badges($section, $icons = false)
{
	...
	$highlightedhtml = $icons ? '<span class="' . $inlflex . '" aria-label="' . $section['highlighted'] . '"><i class="fa fa-lightbulb-o"></i></span>' :
	'<span class="badge badge-pill badge-primary">' . $section['highlighted'] . '</span>';
	$hiddenfromstudentshtml = $icons ? '<span class="' . $inlflex . '" aria-label="' . $section['hiddenfromstudents'] . '"><i class="fa fa-eye-slash"></i></span>' :
	'<span class="badge badge-pill badge-warning">' . $section['hiddenfromstudents'] . '</span>';
	$notavailablehtml = $icons ? '<span class="' . $inlflex . '" aria-label="' . $section['notavailable'] . '"><i class="fa fa-ban"></i></span>' :
	...
	'<span class="restriction' . $inlflex . '" aria-label="' . get_string('accessrestrictions', 'availability') . '"><i class="fa fa-lock"></i></span>' : '';
 ...
 *
 */
function theme_mb2nl_module_avalibility($mod)
{
	...
	// This feature works only with Moodle 4+
	if ( $m4 )
	{	
		$format = course_get_format($COURSE);
		$renderer = $PAGE->get_renderer('format_topics');
 ...
 *
 */
function theme_mb2nl_module_has_restrictions( $mod )
{
	global $CFG, $PAGE, $COURSE;
	...
	if ( theme_mb2nl_module_avalibility( $mod ) )
	{
		return true;
	}
...
function theme_mb2nl_mb2fields_filed($name, $courseid = 0)
{
	...
	foreach ( $fields as $k=>$f )
	{
		...
		elseif( ($name === 'mb2section' || $name === 'mb2promo') && strip_tags( $val ) ) // strip_tags to make sure if there is some content
		{
			return $val;
		}
		else
...
function theme_mb2nl_module_sections( $block = false, $progress = true )
{
	...
	foreach ( $sections as $section )
	{
		...
		$output .= '<div class="coursetoc-section coursetoc-section-' . $section['num'] . $completecls . $isactive . $hiddencls . $highlightedcls . '" data-id="' . $section['id'] . '" data-num="' . $section['num'] . '" data-complete="' . $datacompletepctg . '">';
		$output .= '<div class="coursetoc-section-tite">';
		$output .= '<button type="button" class="coursetoc-section-toggle themereset' . theme_mb2nl_bsfcls(1,'row','between','center') . '" aria-controls="coursetoc-section-modules-' . $section['id'] . '" aria-label="' . $section['name'] . '">';
...
function theme_mb2nl_course_completion_percentage($cc = 0)
{
	...
	$cancomplete = isloggedin() && ! isguestuser();
	$enrolled = ( $iscourse->id != SITEID && is_enrolled( $context, $USER->id, '', true ) );
...
function theme_mb2nl_course_tags_block($id)
{
	...
	$output .= '<h4 class="block-heading">' . get_string('coursetags', 'tag') . '</h4>';	
