	...
	for ($i=1; $i<=10; $i++)
	{
		...
		if (isset($socialName[0]) && $socialName[0] !== '')
		{
			...
			if ( array_key_exists( $socialName[0], $custom ) )
			{
				...
				{
					$iconhtml = $CFG->version < 2023042400 ? '<i class="' . $custom[$socialName[0]] . '"></i>' : '<i class="fa fa-brands fa-' . $socialName[0] . '"></i>';
...
function theme_mb2nl_page_title( $coursename = true, $onlycourse = false )
{
	...
	$title_arr = end( $title_arr );
	$title_arr = explode( '|', $title_arr ); // Required for Moodle 4.2.2+
	...
	if ( $COURSE->id > $SITE->id && theme_mb2nl_is_cmainpage() )
	{
		return format_text( $COURSE->fullname, FORMAT_HTML );
	}

	// Standard title
	if ( $coursename && $COURSE->id > $SITE->id && ! theme_mb2nl_is_cmainpage() )
	{
		if ( $onlycourse )
		{
			return format_text( $COURSE->fullname, FORMAT_HTML );
	...
	$title .= reset( $title_arr ); // The 'reset' is required for Moodle 4.2.2+

	//$info = array('heading' => 'sdsdsdsd', 'user'=>$USER);
	//return $OUTPUT->context_header($info, 1);
...
function theme_mb2nl_course_fields( $courseid, $divs = true )
{
	...
	foreach ( $fields as $f )
	{
		...
		$output .= '<span class="name">'. $icon . format_text( $f['name'], FORMAT_HTML ) . ':</span>';
...
function theme_mb2nl_is_stycky($modal = false)
{
	...
	if ( ! $stickynav || theme_mb2nl_is_login(true) || ( ! $modal && theme_mb2nl_is_custom_enrolment_page() && theme_mb2nl_is_eopn()) )
	{
		return 0;
	}
